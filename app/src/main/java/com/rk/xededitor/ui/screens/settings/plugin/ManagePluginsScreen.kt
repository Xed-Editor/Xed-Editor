            ...
            PreferenceGroup(heading = stringResource(id = R.string.installed)) {
                plugins.forEach { plugin ->
                    var active by remember {
                        mutableStateOf(
                            PluginUtils.isPluginActive(activity, plugin.info.packageName, false)
                        )
                    }
                    PluginRow(plugin.info, true, active) {
                        active = !active
                        coroutineScope.launch(Dispatchers.IO) {
                            PluginUtils.setPluginActive(activity, plugin.info.packageName, active)
                        }
                    }
                }
            }
...
@Composable
fun AvailablePlugins(viewModel: PluginViewModel, onPluginSelected: (PluginInfo) -> Unit) {
        ...
        Column {
            PreferenceGroup(heading = stringResource(id = R.string.available)) {
                viewModel.availablePlugins.forEach { plugin ->
                    PluginRow(plugin, false) { onPluginSelected(plugin) }
                }
            }
        }
    } else {
...
private fun PluginRow(
    ...
    PreferenceTemplate(
        title = { Text(text = plugin.title, style = MaterialTheme.typography.titleMedium) },
        description = {
...
@Composable
fun NoContentScreen(label: String) {
    Column(
        modifier = Modifier.fillMaxSize(),
        verticalArrangement = Arrangement.Center,
        horizontalAlignment = Alignment.CenterHorizontally,
...
@Composable
fun AddPluginDialog(onDismissRequest: () -> Unit, onConfirm: () -> Unit) {
    AlertDialog(
        onDismissRequest = onDismissRequest,
        title = {
            Text(text = "${rkUtils.getString(R.string.add)} ${rkUtils.getString(R.string.plugin)}")
        },
        text = { Text(text = stringResource(R.string.choose_zip)) },
        confirmButton = {
            Button(onClick = onConfirm) { Text(stringResource(R.string.choose_file)) }
        },
        dismissButton = {
...
@Composable
fun DownloadDialog(onDismissRequest: () -> Unit, plugin: PluginInfo, onConfirm: () -> Unit) {
    AlertDialog(
        onDismissRequest = onDismissRequest,
...
private fun getFileName(uri: Uri, activity: Context): String? {
