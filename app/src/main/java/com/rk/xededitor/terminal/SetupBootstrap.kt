...
class SetupBootstrap(val terminal: Terminal, val runnable: Runnable) {
    ...
    fun init() {
        if (
            PreferencesData.getBoolean(PreferencesKeys.FAIL_SAFE, false) ||
                File(terminal.filesDir.parentFile, "root/bin/proot").exists()
        ) {
            runnable.run()
            return
        }
            ...
            val complete = Runnable {
                GlobalScope.launch(Dispatchers.Default) {
                    ...
                    Shell("sh").apply {
                        ...
                        val targetRootfs =
                            File(terminal.filesDir.parentFile, "rootfs").apply {
                                if (exists().not()) {
                                    mkdirs()
                                }
                            }
                        ...
                        // run("echo \"${terminal.filesDir.parentFile!!.absolutePath}/root/bin/proot
                        // -S ${terminal.filesDir.parentFile!!.absolutePath}/rootfs\" >
                        // ${terminal.filesDir.parentFile!!.absolutePath}/proot.sh")
