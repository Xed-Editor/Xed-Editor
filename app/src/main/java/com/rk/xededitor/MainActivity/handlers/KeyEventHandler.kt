      ...
      when (keyEvent.keyCode) {
        KeyEvent.KEYCODE_W -> {
          MainActivity.activityRef.get()
            ?.let { it.adapter.removeFragment(it.tabLayout.selectedTabPosition) }
        }
        ...
        KeyEvent.KEYCODE_K -> {
          MainActivity.activityRef.get()?.let {
            ...
            it.tabLayout.selectTab(it.tabLayout.getTabAt(it.tabLayout.selectedTabPosition - 1))
          }
      ...
      if (isCtrlPressed) {
        when (event.keyCode) {
          ...
          KeyEvent.KEYCODE_G -> {
            val line = 0
            var column =
              (editor.text.getLine(line).length).coerceAtMost(editor.cursor.leftColumn)
            if(column < 0){
              column = 0
            }
          ...
          KeyEvent.KEYCODE_B -> {
            ...
            var column =
              (editor.text.getLine(line).length - 1).coerceAtMost(editor.cursor.leftColumn)
            if(column < 0){
              column = 0
            }
            editor.cursor.set(line,column)
              ...
              currentFragment()?.let {
                ...
                MainActivity.activityRef.get()?.let { activity ->
                  startActivityForResult(
                    activity, intent, REQUEST_CODE_OPEN_DIRECTORY, null
                  )
                }
              }
            }
