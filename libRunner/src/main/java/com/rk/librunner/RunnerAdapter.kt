...
class RunnerAdapter(
	private val runners: List<RunnableInterface>,
	private val dialog:AlertDialog,
	private val onRunnerSelected: (RunnableInterface) -> Unit
	...
	inner class RunnerViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
		...
		fun bind(runner: RunnableInterface) {
			titleView.text = runner.getName()
			descriptionView.text = runner.getDescription()
			iconView.setImageDrawable(runner.getIcon(itemView.context) ?: ContextCompat.getDrawable(itemView.context, R.drawable.android))
