...
object Timeout {
    private val timeouts = ConcurrentHashMap<Timeouts, Int>()

    init {
        Timeouts.values().forEach { timeouts[it] = it.defaultTimeout }
    }

    /**
     * Get the timeout for a request. The [type] is the request type and the result is the timeout
     * in milliseconds
     */
    ...
    /**
     * Set the timeout for a request. The [type] is the request type and the [time] is the timeout
     * in milliseconds
     */
    ...
    fun setTimeouts(loaded: Map<Timeouts, Int>) {
        timeouts.replaceAll { key: Timeouts, value: Int -> loaded[key] ?: value }
    }
}
