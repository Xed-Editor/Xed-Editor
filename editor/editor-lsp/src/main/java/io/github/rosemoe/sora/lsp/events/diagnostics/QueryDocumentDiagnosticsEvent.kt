...
class QueryDocumentDiagnosticsEvent : AsyncEventListener() {
    ...
    override suspend fun handleAsync(context: EventContext) {
        ...
        val future =
            requestManager.diagnostic(editor.uri.createDocumentDiagnosticParams()).thenApply {
                either: DocumentDiagnosticReport ->
                if (either.isRelatedFullDocumentDiagnosticReport)
                    either.relatedFullDocumentDiagnosticReport.relatedDocuments
                else either.relatedUnchangedDocumentDiagnosticReport.relatedDocuments
            }
