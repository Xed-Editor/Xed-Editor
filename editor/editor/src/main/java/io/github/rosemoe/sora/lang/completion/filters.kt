...
class FuzzyScore(
    ...
    companion object {
        ...
        @JvmStatic val default: FuzzyScore = FuzzyScore(-100, 0)
...
data class FuzzyScoreOptions(val firstMatchCanBeWeak: Boolean, val boostFullMatch: Boolean) {
...
fun fuzzyScore(
    ...
    while (patternPos < patternLen) {
        ...
        // Reduce search space to possible matching word positions and to possible access from next
        // row
        ...
        while (wordPos < nextMaxWordMatchPos) {
            ...
            val leftScore =
                if (canComeLeft) table[row][column - 1] + (if (diag[row][column - 1] > 0) -5 else 0)
                else 0 // penalty for a gap start

            val canComeLeftLeft = wordPos > minWordMatchPos + 1 && diag[row][column - 1] > 0
            val leftLeftScore =
                if (canComeLeftLeft)
                    table[row][column - 2] + (if (diag[row][column - 2] > 0) -5 else 0)
                else 0 // penalty for a gap start
            ...
            if (
                ...
                // always prefer choosing left left to jump over a diagonal because that means a
                // match is earlier in the word
    ...
    while (row >= 1) {
        ...
        // Overturn the "forwards" decision if keeping the "backwards" diagonal would give a better
        // match
        if (
            ...
            &&
                patternLow[patternStart + row - 1] ==
                    wordLow[
                        wordStart + column - 1] // only if we can do a contiguous match diagonally
                &&
                !isUpperCaseAtPos(
...
internal fun doScore(
    ...
    if (patternPos == patternStart) { // first character in pattern
        if (wordPos > wordStart) {
            // the first pattern character would match a word character that is not at the word
            // start
        ...
        if (newMatchStart) {
            // this would be the beginning of a new match (i.e. there would be a gap before this
            // location)
            score += if (isGapLocation) 2 else 0
        } else {
            // this is part of a contiguous match, so give it a slight bonus, but do so only if it
            // would not be a preferred gap location
            score += if (isGapLocation) 0 else 1
        }
    }

    if (wordPos + 1 == wordLen) {
        // we always penalize gaps, but this gives unfair advantages to a match that would match the
