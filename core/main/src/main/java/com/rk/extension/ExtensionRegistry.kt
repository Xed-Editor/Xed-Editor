...
object ExtensionRegistry {
    ...
    suspend fun fetchExtensions(): List<ExtensionManifest> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            for (i in 0 until extensions.length()) {
                ...
                for (j in 0 until authors.length()) {
                    ...
                    extensionAuthors.add(
                        ExtensionAuthor(displayName = author, github = null)
                    )
                }
    ...
    suspend fun getIconUrl(manifest: ExtensionManifest): String? = withContext(Dispatchers.IO){
        runCatching {
    ...
    suspend fun getReadmeUrl(manifest: ExtensionManifest):String? = withContext(Dispatchers.IO){
        runCatching {
            val url = URL("https://xed-editor.app/api/extensions/${manifest.id}")
            val connection = url.openConnection() as HttpURLConnection
            connection.requestMethod = "GET"
