package com.rk.extension.internal
...
fun LocalExtension.load(
    ...
    if (isMainThread()) {
        return@run Result.failure(RuntimeException("Attempted to load extension '${info.name}' on the main thread. Extension loading must be performed on a background thread."))
    }
    ...
    if (ExtensionAPI::class.java.isAssignableFrom(mainClassInstance)) {
        val instance = mainClassInstance.getDeclaredConstructor().newInstance() as? ExtensionAPI
            ?: return@run Result.failure(
                RuntimeException("Failed to instantiate main class '${mainClassInstance.name}' for extension '${info.name}'. The class could not be cast to ExtensionAPI. Ensure it implements the ExtensionAPI interface and has a public no-argument constructor.")
            )
        ...
        Result.failure(RuntimeException("The main class '${info.mainClass}' of plugin '${info.name}' does not implement the ExtensionAPI interface. Please ensure the main class correctly implements this interface."))
    }
}

suspend fun ExtensionManager.installExtension(
