...
fun LspServerLogs(server: BaseLspServer, id: String) {
    ...
    val logText = instance?.let { buildLogs(server, it) } ?: strings.instance_not_found.getString()
    LogScreen(logText, "Language Server Error Report", "server_logs")
}

private fun buildLogs(server: BaseLspServer, instance: BaseLspServerInstance): String {
    val entries = instance.logs.joinToString("\n") { "[${it.level.name.uppercase()}] ${it.message}" }
