...
inline fun terminalChecks(): SnapshotStateList<Check> {
    return remember { mutableStateListOf<Check>(
        ...
        Check(
            ...
            run = { printLog ->
                ...
                exitcode == 0 && shell.exists() && shell1.exists() && shell.canRead() && shell1.canRead() && shell.canExecute() && shell1.canExecute()
            }
        ),
        ...
        Check(label = "Check Ubuntu", run = { printLog ->
        ...
        Check(label = "Check Network Access", run = { printLog ->
            if (!isTerminalInstalled()) {
        ...
        Check(label = "Check Abnormalities", run = { printLog ->
            if (!isTerminalInstalled()) {
                printLog("Ubuntu not installed, skipping")
                return@Check true
            }
