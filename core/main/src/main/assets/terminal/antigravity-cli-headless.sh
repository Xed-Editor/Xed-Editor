...
install_agy() {
  log "Installing Antigravity CLI (Termux build)..."
  ...
  mkdir -p "$LOCAL/bin"
  cp "$STAGING_DIR/bin/agy" "$LOCAL/bin/agy" 2>/dev/null || { log "Failed to copy agy"; return 1; }
  ...
  rm -rf "$STAGING_DIR"
  log "Installed agy + agy.va39 to $LOCAL/bin"
}
