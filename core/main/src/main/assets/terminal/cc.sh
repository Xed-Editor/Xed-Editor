...
if [ -n "$missing_packages" ]; then
    echo -e "\e[34;1m[*]\e[0m Installing missing packages: $missing_packages"
...
run_code(){
    echo -e "\e[32;1m[✓]\e[0m Compilation successful! Running...\e[0m"
    chmod +x "$OUTPUT_FILE"
...
if [ -f "$TARGET_FILE" ]; then
    ...
    echo -e "\e[34;1m[*]\e[0m Compiling file: $TARGET_FILE\e[0m"
    if $COMPILER -o "$OUTPUT_FILE" "$TARGET_FILE"; then
        run_code
    else
        echo -e "\e[31;1m[✗]\e[0m Compilation failed!\e[0m"
        exit 1
    fi
...
# Check if the target is a directory (Make or CMake project)
    ...
    if [ -f "$TARGET_FILE/Makefile" ]; then
        echo -e "\e[34;1m[*]\e[0m Building with Makefile\e[0m"
        if make -C "$TARGET_FILE"; then
            OUTPUT_FILE=$(find "$TARGET_FILE" -maxdepth 1 -type f -executable | head -n 1)
            if [ -n "$OUTPUT_FILE" ]; then
                run_code
            else
                echo -e "\e[31;1m[✗]\e[0m No executable found after make.\e[0m"
            fi
        else
            echo -e "\e[31;1m[✗]\e[0m Build failed!\e[0m"
            exit 1
        fi
    elif [ -f "$TARGET_FILE/CMakeLists.txt" ]; then
        echo -e "\e[34;1m[*]\e[0m Building with CMake\e[0m"
        ...
        if cmake .. && make; then
            ...
            if [ -n "$OUTPUT_FILE" ]; then
                ...
                echo -e "\e[31;1m[✗]\e[0m No executable found after cmake build.\e[0m"
            fi
        else
            echo -e "\e[31;1m[✗]\e[0m Build failed!\e[0m"
            exit 1
