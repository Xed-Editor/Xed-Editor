...
class DiagnosticsContainer {
    ...
    private val diagnosticsMap by
        lazy(LazyThreadSafetyMode.NONE) { HashMap<FileUri, MutableList<Diagnostic>>() }
    ...
    fun addDiagnostics(uri: FileUri, diagnostics: List<Diagnostic>) {
        diagnostics.forEach { findOrReplaceDiagnostic(uri, it) }
    }
    ...
    private fun findOrReplaceDiagnostic(uri: FileUri, newDiagnostic: Diagnostic) {
        ...
        findDiagnostic(uri, newDiagnostic.range.start.line, newDiagnostic.range.start.character)
