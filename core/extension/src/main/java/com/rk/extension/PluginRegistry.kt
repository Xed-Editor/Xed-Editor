...
object PluginRegistry {
    ...
    suspend fun fetchExtensions() = withContext(Dispatchers.IO) {
        val extensions = GitHubApi.fetchContents("plugins")
        extensions.filter { it.type == "dir" }.mapNotNull { extDir ->
            ...
            val pluginInfo = try {
                ...
                val decoded = pluginFileInfo.content?.decodeFromBase64()?.decodeToString()
                    ?: fetchRawFile(pluginFileInfo)
                    ?: return@mapNotNull null

                val pluginInfo = Gson().fromJson(decoded, PluginInfo::class.java)

                cache[pluginPath] = CachedPlugin(
    ...
    suspend fun downloadExtension(id: ExtensionId, targetDir: File) = withContext(Dispatchers.IO) {
