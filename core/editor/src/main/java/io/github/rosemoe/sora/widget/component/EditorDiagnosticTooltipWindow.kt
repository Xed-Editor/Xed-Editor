...
open class EditorDiagnosticTooltipWindow(editor: CodeEditor) :
    ...
    private val briefMessageText =
        rootView.findViewById<TextView>(R.id.diagnostic_tooltip_brief_message)
    private val detailMessageText =
        rootView.findViewById<TextView>(R.id.diagnostic_tooltip_detailed_message)
    private val quickfixText =
        rootView.findViewById<TextView>(R.id.diagnostic_tooltip_preferred_action)
    private val moreActionText =
        rootView.findViewById<TextView>(R.id.diagnostic_tooltip_more_actions)
    ...
    init {
        ...
        moreActionText.setText(
            I18nConfig.getResourceId(R.string.sora_editor_diagnostics_more_actions)
        )
    ...
    private fun registerEditorEvents() {
                    ...
                    MotionEvent.ACTION_HOVER_MOVE -> {
                        if (!(popupHovered || menuShown)) {
                            if (editor.isScreenPointOnText(e.x, e.y)) {
                                if (
                                    abs(e.x - lastHoverPos.first) > ViewUtils.HOVER_TAP_SLOP ||
                                        abs(e.y - lastHoverPos.second) > ViewUtils.HOVER_TAP_SLOP
                                ) {
                                    updateLastHover()
                                    val pos = editor.getPointPositionOnScreen(e.x, e.y)
                                    hoverPosition =
                                        editor.text.indexer.getCharPosition(
        ...
        eventManager.subscribeEvent<ColorSchemeUpdateEvent> { _, _ -> applyColorScheme() }
        eventManager.subscribeEvent<EditorFocusChangeEvent> { event, _ ->
            if (!event.isGainFocus) {
                dismiss()
            }
        }
        eventManager.subscribeEvent<EditorReleaseEvent> { _, _ -> isEnabled = false }
    }
    ...
    protected open fun updateDiagnostic(pos: CharPosition) {
        ...
        if (diagnostics != null) {
            ...
            if (diagnosticList.isNotEmpty()) {
                ...
                if (!editor.getComponent<EditorAutoCompletion>().isCompletionInProgress) show()
            } else {
                updateDiagnostic(null, null)
            }
    ...
    protected open fun applyColorScheme() {
        ...
        briefMessageText.setTextColor(
            colorScheme.getColor(EditorColorScheme.DIAGNOSTIC_TOOLTIP_BRIEF_MSG)
        )
        detailMessageText.setTextColor(
        ...
        moreActionText.setTextColor(
            colorScheme.getColor(EditorColorScheme.DIAGNOSTIC_TOOLTIP_ACTION)
        )
    ...
    protected open fun isSelectionVisible(): Boolean {
        ...
        return buffer[0] >= editor.offsetY &&
            ...
            buffer[1] - 100f /* larger than a single character */ <= editor.offsetX + editor.width
    }
    ...
    protected open fun updateWindowSize() {
        ...
        if (quickfixPanel.visibility == View.VISIBLE) {
            quickfixPanel.measure(
                ...
                MeasureSpec.makeMeasureSpec(114514, MeasureSpec.AT_MOST),
            )
