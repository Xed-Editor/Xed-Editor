...
/**
 * Called when the editor is going to be released. That's when [CodeEditor.release] is called. You
 * may subscribe this event to release resources when you are holding editor-specific resources.
 *
...
/**
 * Event for ime private command execution. When
 * [android.view.inputmethod.InputConnection.performPrivateCommand] is called, this event will be
 * triggered. You can subscribe to this event in order to interact with your own inputmethod and
 * thus implement specific features between this editor and your IME app.
 *
...
/**
 * This event is triggered when editor is building its [EditorInfo] object for IPC. You can
 * customize the info to add extra information for the ime, due to implement specific features
 * between this editor and your own IME. But note that [EditorInfo.inputType],
 * [EditorInfo.initialSelStart], [EditorInfo.initialSelEnd] and [EditorInfo.initialCapsMode] should
 * not be modified. They are managed by editor self.
 *
 * @author Rosemoe
 */
 ...
 */
class TextSizeChangeEvent(editor: CodeEditor, val oldTextSize: Float, val newTextSize: Float) :
    Event(editor)

/**
 * Event when search result is available in main thread. Note that this event is also triggered when
 * query is changed to null.
 *
 * @author Rosemoe
 */
